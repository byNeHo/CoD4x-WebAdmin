...
module.exports = {
	RconConsoleCommand: function(req, res, next) {
		BluebirdPromise.props({
			...
			getserver: Servers.findOne({'_id':req.params.id, rcon_password: { $gt: [] }}).execAsync(),
	...
	RconPermbanNoImage: function(req, res, next) {
		BluebirdPromise.props({
			servers: Servers.findOne({'admins_on_server':req.user._id, '_id':req.params.id, rcon_password: { $gt: [] }}).execAsync(),
			getcommand: Rconcommand.findOne({'command_name': 'permban'}).execAsync(),
	...
	RconUnban: function(req, res, next) {
		BluebirdPromise.props({
			...
			if (results.getban){
				Servers.findOne({'slug_name':results.getban.server_name, 'admins_on_server':req.user._id, 'rcon_password': { $gt: [] }})
					.then(function(server_admins) {
						if (server_admins){
	...
	RconUnbanTempban: function(req, res, next) {
		...
		BluebirdPromise.props({
			...
			if (results.getban){
				Servers.findOne({'slug_name':results.getban.game_server, 'admins_on_server':req.user.id, 'rcon_password': { $gt: [] }})
					.then(function(server_admins) {
						if (server_admins){
							if (req.user.local.user_role >= results.requiredpower.minimum_power_for_player_unban){
								var	rcon = require('srcds-rcon')({address:server_admins.ip+':'+server_admins.port,password: server_admins.rcon_password});
